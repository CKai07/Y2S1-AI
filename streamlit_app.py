...
try:
    ...
    st.warning("Matplotlib is not installed. Some visualizations may be limited.")
...
try:
    ...
    st.warning("Plotly is not installed. Using alternative visualization methods.")
...
def display_historical_test(predictor):
    """Display a historical test of the prediction accuracy"""
    st.subheader("Historical Prediction Accuracy Test")
    
    col1, col2 = st.columns(2)
    with col1:
        ticker = st.text_input("Enter ticker symbol for historical test", "AAPL")
    with col2:
        days = st.number_input("Number of days to test", min_value=5, max_value=60, value=30, step=5)
    
    if st.button("Run Historical Test"):
        with st.spinner(f"Testing prediction accuracy for {ticker} over the last {days} days..."):
            try:
                # Set the ticker and run historical test
                predictor.set_ticker(ticker)
                results = predictor.test_historical_accuracy(days=days)
                
                # Display metrics
                col1, col2, col3 = st.columns(3)
                col1.metric("RMSE", f"${results['rmse']:.2f}")
                col2.metric("MAE", f"${results['mae']:.2f}")
                ...
                df_results = pd.DataFrame({
                    ...
                    'Predicted Price': results['predicted_prices'],
                ...
                try:
                    ...
                    ax.plot(results['dates'], results['actual_prices'], label='Actual Price')
                    ax.plot(results['dates'], results['predicted_prices'], label='Predicted Price')
                    ax.set_title(f"Historical Prediction Accuracy for {ticker}")
                    ax.set_xlabel("Date")
                    ax.set_ylabel("Price ($)")
                    ax.legend()
                    st.pyplot(fig)
                    ...
                    ax2.set_title(f"Prediction Errors for {ticker}")
                    ...
                    # If matplotlib fails, use Streamlit's native charting
                    st.line_chart(df_results[['Actual Price', 'Predicted Price']])
                    st.bar_chart(df_results['Error'])
                
                st.success(f"Completed historical test for {ticker} with RMSE: ${results['rmse']:.2f}")
                
            except Exception as e:
                st.error(f"Error in historical test: {str(e)}")
                st.info("This could be due to insufficient historical data or API limitations. Try a different ticker or fewer days.")
...
def display_home_page():
    ...
    st.header("Welcome to the Stock Market Prediction AI")
    
    # Introduction
    st.markdown("""
    This application helps you predict stock prices using machine learning algorithms and technical indicators.
    
    ### Features:
    - **Single Stock Prediction**: Get price predictions for a specific stock
    - **Stock Comparison**: Compare predictions for multiple stocks
    - **Historical Accuracy Test**: Evaluate the prediction accuracy using historical data
    
    ### How it works:
    The system uses a combination of LSTM neural networks and Random Forest models trained on historical price data
    and technical indicators to generate predictions with confidence levels.
    """)
    
    # Quick demo section
    st.subheader("Quick Demo")
    demo_ticker = st.text_input("Enter a ticker symbol to try it out:", "AAPL")
    if st.button("Get Quick Prediction"):
        with st.spinner(f"Analyzing {demo_ticker}..."):
            try:
                predictor = StockPredictor()
                predictor.set_ticker(demo_ticker)
                prediction = predictor.predict_with_confidence(force_update=True)
                ...
                col1.metric(
                    ...
                    delta=None,
                    help=f"Last updated: {prediction['last_updated']}"
                )
                
                col2.metric(
                    "Predicted Price", 
                    f"${prediction['predicted_price']:.2f}",
                    delta=f"{((prediction['predicted_price'] - prediction['current_price']) / prediction['current_price'] * 100):.2f}%"
                )
                ...
                st.info(f"**Recommendation**: {prediction['action'].upper()}")
                
                # Display confidence levels with a progress bar
                st.subheader("Prediction Confidence")
                
                for level, value in prediction['confidence_levels'].items():
                    st.progress(value)
                    st.caption(f"{level.title()}: {value*100:.1f}%")
...
def display_about_page():
    ...
    st.header("About Stock Market Prediction AI")
    
    st.markdown("""
    ### How It Works
    
    This application uses advanced machine learning techniques to predict stock price movements:
    
    1. **Data Collection**: Historical stock data is retrieved from reliable financial APIs
    ...
    3. **Machine Learning**: Two models are trained and combined:
       - Long Short-Term Memory (LSTM) neural networks
       - Random Forest regression
       
    4. **Prediction with Confidence**: The system provides not just a price prediction, but confidence levels and recommended actions
    
    ### Limitations
    
    - Stock markets are influenced by countless factors, many unpredictable
    - The system relies on technical analysis and may miss fundamental or news-based catalysts
    - Predictions are more uncertain when markets are closed
    - This is an educational tool, not financial advice
    
    ### Technologies Used
    
    - Python for data processing and machine learning
    - TensorFlow and Keras for LSTM neural networks
    ...
    Stock data is fetched using Yahoo Finance API.
    """)
            ...
            for i, ticker in enumerate(tickers):
                ...
                try:
                    ...
                    results.append({
                        ...
                        'Strong Confidence': f"{prediction['confidence_levels'].get('Strong', 0) * 100:.1f}%",
